...
/**
 * Script para inserir as questões estratégicas (versão adaptada à estrutura atual)
 */
...
const strategicQuestionsData = [
  ...
  },
  {
    order_index: 16,
    type: "text",
    title: "Pense no quanto você já gastou com roupas que não usa ou que não representam quem você é... Você acredita que ter acesso a um material estratégico, direto ao ponto, que te ensina a aplicar seu estilo com clareza, faria diferença?",
    options: [
...
const insertStrategicQuestions = async () => {
  ...
  try {
    ...
    for (const questionData of strategicQuestionsData) {
      // Inserir/atualizar questão usando apenas as colunas disponíveis
    ...
    console.log("🎉 Todas as questões estratégicas foram inseridas com sucesso!");
    return true;
  } catch (error) {
