...
requiredScripts.forEach(script => {
  ...
  console.log(`${exists ? '✅' : '❌'} ${script}: ${exists ? 'OK' : 'MISSING'}`);
});
...
try {
  ...
  const lovableScripts = Object.keys(packageJson.scripts || {}).filter(key => key.startsWith('lovable:'));
...
try {
  ...
  console.log(`${gitStatus.trim() ? '⚠️' : '✅'} Git status: ${gitStatus.trim() ? 'Há alterações pendentes' : 'Limpo'}`);
  console.log('📍 Remotes:');
