...
type StyleCategory = keyof typeof styleConfig;
...
const PrimaryStyleCard: React.FC<PrimaryStyleCardProps> = ({
  ...
  // Corrige o acesso ao styleConfig para evitar erro de índice string
  const category = primaryStyle.category as StyleCategory;
  const imageUrl = customImage || (styleConfig[category]?.image || '');
  const description = customDescription || (styleConfig[category]?.description || 'Descrição do estilo não disponível');
