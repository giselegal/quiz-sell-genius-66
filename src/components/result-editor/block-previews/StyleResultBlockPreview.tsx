...
const StyleResultBlockPreview: React.FC<StyleResultBlockPreviewProps> = ({
  ...
  const imageUrl = content.customImage || (styleConfig[primaryStyle.category]?.image || '');
  const guideImageUrl = styleConfig[primaryStyle.category]?.guideImage || '';
  const description = content.description || (styleConfig[primaryStyle.category]?.description || 'Descrição do estilo não disponível');
