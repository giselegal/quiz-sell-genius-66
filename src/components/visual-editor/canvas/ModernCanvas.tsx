    ...
    return (
      <div
        ...
        className={`
          ...
          ${
            isSelected
              ? "ring-2 ring-blue-500 bg-blue-50 rounded-md"
              : "hover:bg-gray-50 rounded-md"
          }
          ${
            !isPreviewMode
              ? "border border-dashed border-transparent hover:border-blue-300"
              : ""
          }
        `}
                ...
                <div className="flex items-center justify-between">
                  <span
                    dangerouslySetInnerHTML={{
                      __html: option.text || `Opção ${index + 1}`,
                    }}
                  />
                </div>
              </button>
      ...
      case "result-title":
        return (
          <h1 className="text-4xl font-bold text-center text-blue-600 my-6">
            {element.content.dynamicTitle
              ? element.content.placeholder
              : element.content.title || "Seu Estilo Descoberto"}
          </h1>
        );
      ...
      case "result-subtitle":
        return (
          <p className="text-lg text-center text-gray-600 my-4">
            {element.content.dynamicDescription
              ? element.content.placeholder
              : element.content.text || "Descrição do seu estilo..."}
          </p>
        );
        ...
        return (
          <div className="bg-yellow-50 border border-yellow-200 p-6 rounded-lg text-center my-6">
            ...
            <div className="text-sm font-medium text-red-600">
              {element.content.discount || "60% OFF"} -{" "}
              {element.content.urgency || "Oferta por tempo limitado"}
            </div>
          </div>
        );
      ...
      default:
        return (
          <div className="p-4 bg-gray-100 rounded-md">
            <p className="text-sm text-gray-600">Componente: {element.type}</p>
          </div>
        );
    }
  };
      ...
      <ScrollArea className="h-full">
        <div className={`min-h-full p-6 ${getViewportClass()}`}>
          ...
          {sortedElements.length > 0 ? (
            <div className="space-y-2">{sortedElements.map(renderElement)}</div>
          ) : (
            <div className="flex items-center justify-center h-64 border-2 border-dashed border-gray-300 rounded-lg">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-400">
                  {!isPreviewMode
                    ? "Adicione componentes da barra lateral ou aguarde o carregamento automático"
