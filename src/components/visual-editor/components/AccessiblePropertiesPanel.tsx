...
import { QuizComponentProps, OptionChoice } from "../../../hooks/useQuizEditorState";
...
const AccessiblePropertiesPanel: React.FC<AccessiblePropertiesPanelProps> = React.memo(({
  selectedComponent,
  onPropsUpdate,
}) => {
  ...
  // Validação de props
  const validateProps = useCallback((key: string, value: unknown): ValidationError[] => {
    ...
    switch (key) {
      ...
      case "height":
        if (typeof value === "number" && (value < 1 || value > 1000)) {
          errors.push({ field: key, message: "Altura deve estar entre 1 e 1000 pixels" });
        }
        break;
      case "maxSelections":
        if (typeof value === "number" && value < 1) {
          errors.push({ field: key, message: "Máximo de seleções deve ser pelo menos 1" });
        }
        break;
      case "minSelections":
        if (typeof value === "number" && value < 1) {
          errors.push({ field: key, message: "Mínimo de seleções deve ser pelo menos 1" });
        }
        break;
    }
  ...
  const updateChoiceImage = useCallback((choiceIndex: number, imageUrl: string) => {
    if (!selectedComponent) return;
  ...
  const updateChoiceScore = useCallback((choiceIndex: number, score: number) => {
    if (!selectedComponent) return;
    const choices = [...(selectedComponent.props.choices || [])];
  ...
  const addChoice = useCallback(() => {
    ...
    updateProp("choices", [...(selectedComponent.props.choices || []), newChoice]);
  }, [selectedComponent, updateProp]);

  const removeChoice = useCallback((index: number) => {
    if (!selectedComponent) return;
    const newChoices = selectedComponent.props.choices?.filter((_, i) => i !== index);
    updateProp("choices", newChoices);
  }, [selectedComponent, updateProp]);

  // Renderização de seção
  const renderSection = useCallback((title: string, key: string, content: React.ReactNode) => {
    ...
    return (
      <div className={`form-field ${hasError ? "error" : ""}`}>
        ...
        {React.cloneElement(element as React.ReactElement, {
          id,
          "aria-describedby": `${description ? `${id}-desc ` : ""}${hasError ? `${id}-error` : ""}`.trim(),
    ...
    <>
      {(type === "heading" || type === "text") && 
        renderFormField(
          ...
          <textarea
            ...
            rows={3}
            placeholder={`Digite o ${type === "heading" ? "título" : "texto"}...`}
          />,
          true,
      ...
      {type === "image" && (
        <>
          ...
          {props.src && (
            <div className="image-preview" role="img" aria-label="Preview da imagem">
            ...
            <div key={index} className="option-editor" role="listitem">
              ...
              {renderFormField(
                ...
                <input
                  ...
                  onChange={(e) => updateChoiceScore(index, parseInt(e.target.value) || 0)}
                />,
                false,
  ...
  return (
    <div className="properties-panel">
      ...
      <div className="component-info">
        <span className="component-type" aria-label={`Tipo de componente: ${typeLabel}`}>
          {typeLabel}
        </span>
      </div>
      ...
      <div className="properties-content" role="tablist">
        ...
        {type === "options" && renderSection("Opções de Resposta", "options", renderOptionsProps())}
      </div>
    </div>
  );
});
