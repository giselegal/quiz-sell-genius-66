...
import { trackFunnelEvent, getCurrentFunnelConfig } from '../services/pixelManager';
...
export const useQuizPixel = () => {
  ...
  const trackCTAClick = useCallback((ctaPosition: string) => {
    if (isQuizPage) {
  ...
  const trackScroll = useCallback((percentage: number) => {
    if (isQuizPage && (percentage === 25 || percentage === 50 || percentage === 75 || percentage === 90)) {
