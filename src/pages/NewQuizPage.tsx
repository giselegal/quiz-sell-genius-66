...
export const NewQuizPage: React.FC = () => {
  ...
  const handleButtonClick = useCallback((action: string) => {
    ...
    if (action === "next") {
      ...
      const quizElement = currentStep?.elements.find(el => el.type === "quiz-options");
      const selectedOption = quizElement?.content.options?.[optionIndex];
      
      if (selectedOption) {
        saveAnswer(questionKey, selectedOption.text, selectedOption.points || 0);
        console.log(`Resposta salva: ${selectedOption.text} (${selectedOption.points} pontos)`);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-zinc-900 via-purple-900 to-violet-900 flex items-center justify-center p-4">
        <div className="max-w-md text-center space-y-6">
          ...
          <h1 className="text-2xl font-bold text-white">Quiz não configurado</h1>
          <p className="text-zinc-300">
            {error || "Nenhuma configuração de quiz foi encontrada. Configure o quiz no editor primeiro."}
          </p>
    ...
    <div className="min-h-screen bg-gradient-to-br from-zinc-900 via-purple-900 to-violet-900">
        ...
        <div className="flex justify-between items-center mt-12 pt-8 border-t border-zinc-800">
          ...
          <div className="text-center">
            <p className="text-zinc-400 text-sm">
              Pontuação atual: <span className="text-purple-400 font-bold">{config.score}</span>
            </p>
          </div>
          ...
          >
            {config.currentStepIndex === config.totalSteps - 1 ? "Finalizar" : "Próximo"}
      ...
      {process.env.NODE_ENV === "development" && (
        <div className="fixed bottom-4 right-4 bg-black/80 text-white p-2 rounded text-xs">
          <div>Etapa: {config.currentStepIndex + 1}/{config.totalSteps}</div>
