...
const testHotmartIntegration = async () => {
  ...
  try {
    ...
    for (let i = 0; i < testSales.length; i++) {
      ...
      const response = await fetch('http://localhost:8083/api/hotmart/webhook', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        console.log(`❌ Erro ao processar venda ${sale.transactionId}:`, response.status);
      }
    ...
    const metricsResponse = await fetch('http://localhost:8083/api/analytics/metrics');
    ...
    console.log('🔍 Procure pelos badges "Dados Reais da Hotmart" nas páginas de Overview e Analytics.');
